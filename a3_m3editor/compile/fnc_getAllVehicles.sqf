private ['_types', '_classes', '_cfgvehicles'];
_types = [];
_classes = [];
...
for '_j' from 0 to (count _cfgvehicles)-1 do
{
	...
	try {
		...
		if (_class in M3E_ignoreClasses) then {throw format['%1 is ignored class', _class];};
		if (_type in M3E_ignoreTypes) then {throw format['%1 is ignored type', _type];};
